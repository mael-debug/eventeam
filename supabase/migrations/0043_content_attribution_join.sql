...
--
-- Diagnostiqué en rejouant recompute_account() directement (pg_stat_activity :
-- actif plus de 3 min sans wait_event — CPU-bound, pas bloqué) : l'étape 7
-- (content_attribution) calculait retained_in_window par une sous-requête
-- CORRÉLÉE, réexécutée une fois PAR LIGNE de content (jusqu'à 4313 fois ici),
-- chacune scannant follower_states pour ce compte. Migration 0042 a ajouté
-- l'index manquant sur l'expression de date utilisée par le filtre, sans
-- suffire seule : le nombre de sous-requêtes exécutées (une par publication)
-- restait le problème de fond, pas seulement le coût de chacune.
--
-- Fix : retained_in_window devient une jointure explicite (content x
-- follower_states, agrégée une seule fois par publication) au lieu d'une
-- sous-requête par ligne — même filtre exact (account_id, episode = 1,
-- status <> 'likely_rename', fenêtre de date), même résultat (0 pour une
-- publication sans arrivée dans la fenêtre, via LEFT JOIN + coalesce), mais
-- un seul passage sur follower_states au lieu de 4313.
--
-- Reste identique au reste de la fonction (copie intégrale de
-- pg_get_functiondef, seule l'étape 7 change) — aucun autre comportement
-- touché.
...
begin
  ...
  --
  -- retained_in_window calculé par JOIN + GROUP BY (migration 0043), pas
  -- par sous-requête corrélée par ligne de content : sur un compte à
  -- plusieurs milliers de publications, une sous-requête par ligne
  -- rescanne follower_states autant de fois qu'il y a de publications —
  -- c'est ce qui faisait dépasser le statement_timeout. Même filtre exact,
  -- même résultat (0 via coalesce pour une publication sans arrivée dans
  -- la fenêtre), un seul passage sur follower_states.
  -- ==========================================================
  with daily as (
    select (followed_at at time zone 'UTC')::date as day, count(distinct profile_id) as arrivals
  ...
  retained as (
    select c.id as content_id,
      count(*) filter (where fs.status = 'present') as retained_in_window
    from public.content c
    join public.follower_states fs
      on fs.account_id = c.account_id
     and fs.episode = 1
     and fs.status <> 'likely_rename'
     and (fs.followed_at at time zone 'UTC')::date between c.published_at::date and (c.published_at + interval '48 hours')::date
    where c.account_id = p_account_id
    group by c.id
  ),
  pubs as (
    select c.id as content_id, c.published_at,
      ...
      coalesce(r.retained_in_window, 0) as retained_in_window
    from public.content c
    left join retained r on r.content_id = c.id
    where c.account_id = p_account_id
  )
