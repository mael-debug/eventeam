-- Community Intelligence — deux correctifs distincts pour recompute_account().
--
-- 1. Réglage autovacuum sur les tables vidées et rechargées en masse à
--    chaque import (DELETE puis INSERT du même ordre de grandeur de lignes
--    pour le compte concerné). Le comportement par défaut d'autovacuum
--    (vacuum_scale_factor = 0.2, analyze_scale_factor = 0.1) suffit pour un
--    usage classique où les lignes changent progressivement, mais pas pour
--    ce motif : après un DELETE+INSERT complet, la table peut rester gonflée
--    (tuples morts non encore réclamés) si un nouvel import démarre avant
--    qu'autovacuum n'ait eu le temps de passer — observé concrètement :
--    follower_states remonté de 141 à 361 octets/ligne en quelques heures
--    d'utilisation normale, pas seulement pendant des tests répétés. On
--    abaisse les seuils pour que le vacuum/analyze se déclenche presque
--    immédiatement après un import plutôt que d'attendre 10-20% du volume
--    de la table (qui, pour un compte isolé dans une table multi-comptes,
--    peut ne jamais être atteint alors que CE compte est déjà entièrement
--    remplacé).
--
-- 2. ANALYZE explicite sur follower_states après son remplissage (étape 1),
--    à l'intérieur même de recompute_account(). Aucun autoanalyze ne peut
--    s'exécuter au milieu d'une transaction : les étapes suivantes (3, 6, 7,
--    8...) qui filtrent/groupent follower_states voient donc les
--    statistiques d'AVANT le DELETE+INSERT de cette même transaction.
--
--    Vérifié par un test contrôlé, différent de celui qui avait invalidé
--    cette même piste pour la version 0043 : là, le JOIN content x
--    follower_states sur une condition BETWEEN paramétrée n'avait
--    structurellement aucune alternative au nested loop (pas de clé
--    d'équi-jointure), donc l'ANALYZE ne changeait rien à l'algorithme
--    choisi. Ici, la requête (0044) ne joint plus follower_states : elle
--    construit daily/daily_present par GROUP BY direct dessus (ces CTE sont
--    inlinées par le planner, pas matérialisées — confirmé par le plan qui
--    scanne directement follower_states, jamais une "CTE Scan" sur un objet
--    séparé). Avec des statistiques indiquant ~1 ligne pour ce compte (état
--    d'avant le DELETE+INSERT de la transaction en cours), le planner choisit
--    follower_states_account_id_status_idx (non trié par date) et doit
--    ensuite trier en clair l'intégralité des lignes réelles (jusqu'à 95k)
--    pour le GROUP BY. Avec un ANALYZE explicite juste avant, le planner
--    reconnaît le vrai volume et bascule sur
--    follower_states_account_id_followed_date_idx (index 0042, trié par
--    date), ce qui transforme le tri complet en Incremental Sort quasi
--    gratuit. Coût planifié observé : 11 550 (mauvais choix d'index) contre
--    2 445 639 (bon choix, coût réel plus élevé mais exécution effectivement
--    plus rapide) — la bascule d'index, pas la taille du coût affiché, est
--    ce qui compte ici.
--
--    Ce correctif reste néanmoins un palliatif : recompute_account() rejoue
--    l'intégralité de follower_observations à chaque exécution, donc sa
--    durée croît avec l'historique cumulé (~45 s supplémentaires par import
--    empilé sur ce compte). Le déplacement du recalcul hors du chemin
--    synchrone (job planifié + statut "en cours" + rejouabilité) reste
--    l'action structurelle à traiter ensuite.

alter table public.follower_states set (autovacuum_vacuum_scale_factor = 0.02, autovacuum_analyze_scale_factor = 0.02);
alter table public.following_states set (autovacuum_vacuum_scale_factor = 0.02, autovacuum_analyze_scale_factor = 0.02);
alter table public.cohorts set (autovacuum_vacuum_scale_factor = 0.02, autovacuum_analyze_scale_factor = 0.02);
alter table public.cohort_survival set (autovacuum_vacuum_scale_factor = 0.02, autovacuum_analyze_scale_factor = 0.02);
alter table public.hazard_curve set (autovacuum_vacuum_scale_factor = 0.02, autovacuum_analyze_scale_factor = 0.02);
alter table public.content_attribution set (autovacuum_vacuum_scale_factor = 0.02, autovacuum_analyze_scale_factor = 0.02);
alter table public.acquisition_spikes set (autovacuum_vacuum_scale_factor = 0.02, autovacuum_analyze_scale_factor = 0.02);
alter table public.cross_analyses set (autovacuum_vacuum_scale_factor = 0.02, autovacuum_analyze_scale_factor = 0.02);
...
begin
  ...
  -- ANALYZE explicite : voir l'en-tête de cette migration (0045). Aucun
  -- autoanalyze ne peut s'exécuter au milieu de cette transaction ; sans ce
  -- rafraîchissement, les étapes suivantes voient les statistiques d'avant
  -- le DELETE+INSERT ci-dessus et choisissent un index non trié par date
  -- pour les agrégations par jour, forçant un tri complet au lieu d'un
  -- Incremental Sort quasi gratuit sur l'index 0042.
  analyze public.follower_states;
  ...
  -- ==========================================================
  -- 6. acquisition_spikes — recalculée en entier, sans exception : plus de
  -- budget_eur à préserver ici (cf. public.manual_entries).
  -- ==========================================================
  select
  ...
  insert into public.acquisition_spikes (
    ...
    night_share, signal_share, retention_rate, inferred_type, inference_confidence
  )
  select
  ...
  -- retained_in_window / retention_rate suivent la même méthode que
  -- acquisition_spikes.retention_rate (0017) : part des arrivées de la
  -- fenêtre encore au statut 'present' aujourd'hui.
  --
  -- retained_in_window : voir 0044 — daily_present pré-agrège une fois par
  -- jour (episode=1, status='present'), une sous-requête corrélée par
  -- publication sur cette petite table, jamais un JOIN direct sur
  -- follower_states. L'ANALYZE de l'étape 1 (voir 0045) est ce qui garantit
  -- que le planner choisisse le bon index pour construire cette agrégation.
  -- ==========================================================
  with daily as (
    select (followed_at at time zone 'UTC')::date as day, count(distinct profile_id) as arrivals
  ...
  daily_present as (
    select (followed_at at time zone 'UTC')::date as day, count(*) as present_arrivals
    from public.follower_states
    where account_id = p_account_id and episode = 1 and status = 'present'
    group by (followed_at at time zone 'UTC')::date
  ),
  pubs as (
    select c.id as content_id, c.published_at,
      ...
      (select coalesce(sum(dp.present_arrivals), 0) from daily_present dp
        where dp.day between c.published_at::date and (c.published_at + interval '48 hours')::date) as retained_in_window
  ...
  select
    ...
    p.retained_in_window,
    round(p.retained_in_window::numeric / nullif(p.arrivals_48h, 0), 4),
  ...
  if not exists (select 1 from public.content where account_id = p_account_id) then
    ...
    insert into public.cross_analyses (account_id, import_id, code, dimension, payload, sample_size, confidence, confidence_reason)
    values
      (p_account_id, v_latest_import, 'territory_retention', '', '{}'::jsonb, 0, 'insuffisant', 'nécessite une classification du contenu par territoire éditorial (thème, mise en scène), jamais construite'),
      (p_account_id, v_latest_import, 'cadence_vs_churn', '', '{}'::jsonb, 0, 'insuffisant', 'pas encore implémenté');

    -- format_retention (§3.3) : media_type (post/reel/story) est une donnée
    -- réelle, contrairement au territoire éditorial ci-dessus — calcul
    -- direct à partir de content_attribution.retention_rate (0038).
    insert into public.cross_analyses (account_id, import_id, code, dimension, payload, sample_size, confidence, confidence_reason)
    select
      p_account_id, v_latest_import, 'format_retention', bf.media_type,
      jsonb_build_object('retention_moyenne', round(bf.avg_retention, 4)),
      bf.n,
      case when bf.n >= 20 then 'indicatif' else 'insuffisant' end,
      case when bf.n < 20 then 'moins de 20 publications attribuées pour ce format (n = ' || bf.n || ')' end
    from (
      select c.media_type, avg(ca.retention_rate) as avg_retention, count(*) as n
      from public.content_attribution ca
      join public.content c on c.id = ca.content_id
      where ca.account_id = p_account_id and ca.retention_rate is not null
