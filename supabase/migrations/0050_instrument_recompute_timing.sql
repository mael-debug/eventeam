-- Instrumentation temporaire (§4.10) : la matérialisation de
-- daily_present (0049) a réduit la durée du recalcul sur "Eden Park All
-- All 2" (24min → toujours >19min lors du run suivant) sans la ramener
-- au niveau de référence (~41s sur "Eden Park All"). Il reste donc un
-- second coût non identifié. Plutôt que de deviner lequel des 9 étapes
-- est en cause, on ajoute un RAISE LOG avec clock_timestamp() à chaque
-- frontière de section : un seul run donnera la durée exacte de chacune,
-- à lire ensuite dans les logs Postgres du projet. Purement diagnostique
-- — retirer ces RAISE LOG une fois la vraie cause isolée et corrigée.
