...
-- Postgres n'autorise pas les agrégats à ensemble ordonné (percentile_cont)
-- avec OVER : "OVER is not supported for ordered-set aggregate
-- percentile_cont". Remplacé par une sous-requête corrélée sur la fenêtre
-- glissante de 14 jours (§4.9), équivalente mais compatible.
...
begin
  ...
  -- §4.9 : médiane glissante sur 14 jours (sous-requête corrélée — un
  -- agrégat à ensemble ordonné comme percentile_cont ne peut pas être
  -- utilisé avec OVER en Postgres).
  with daily as (
