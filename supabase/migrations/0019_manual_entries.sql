-- Community Intelligence — généralisation du motif "saisie manuelle".
--
-- Retour explicite sur la correction de 0017/0018 : préserver
-- acquisition_spikes.budget_eur à travers un recalcul intégral en le
-- recopiant via une table temporaire fonctionnait, mais c'est une rustine —
-- une saisie humaine ne devrait jamais vivre dans une colonne d'une table
-- que recompute_account() vide et repeuple, même prudemment. La bonne
-- forme : une table dédiée, jamais touchée par recompute_account, avec une
-- clé métier stable (pas une PK technique de la table recalculée, qui
-- disparaît et se recrée à chaque passage), jointe à la lecture.
--
-- public.manual_entries généralise ce motif à toute saisie humaine future
-- (budgets de pics aujourd'hui ; annotations de catalogue, notes
-- éditoriales, etc. demain) plutôt que d'ajouter une colonne ad hoc à
-- chaque nouveau besoin.

create table public.manual_entries (
  id           uuid primary key default gen_random_uuid(),
  account_id   uuid not null references public.instagram_accounts(id) on delete cascade,
  entity_type  text not null,  -- 'acquisition_spike' aujourd'hui ; 'feature_catalog', etc. demain
  entity_key   text not null,  -- clé métier stable de l'entité annotée (ex. spike_start en texte)
  field        text not null,  -- 'budget_eur', 'note', ...
  value_numeric numeric,
  value_text    text,
  updated_by    uuid references auth.users(id),
  updated_at    timestamptz not null default now(),
  unique (account_id, entity_type, entity_key, field)
);

create index on public.manual_entries (account_id, entity_type);

alter table public.manual_entries enable row level security;

create policy "manual_entries_read" on public.manual_entries
...
create policy "manual_entries_update" on public.manual_entries
...
-- acquisition_spikes redevient une table entièrement recalculée, sans
-- exception : budget_eur et les deux colonnes générées qui en dépendaient
-- sortent de la table. Le coût par abonné conservé (§3.8) se lit désormais
-- via la vue ci-dessous, qui joint le budget saisi à la volée.
drop policy if exists "acquisition_spikes_update_budget" on public.acquisition_spikes;
alter table public.acquisition_spikes drop column if exists cout_brut_eur;
alter table public.acquisition_spikes drop column if exists cout_retenu_eur;
alter table public.acquisition_spikes drop column if exists budget_eur;

create view public.acquisition_spikes_with_budget as
select
  asp.*,
  me.value_numeric as budget_eur,
  case when asp.volume > 0 and me.value_numeric is not null
    then round(me.value_numeric / asp.volume, 2)
  end as cout_brut_eur,
  case when asp.volume > 0 and asp.retention_rate is not null and asp.retention_rate > 0 and me.value_numeric is not null
    then round(me.value_numeric / (asp.volume * asp.retention_rate), 2)
  end as cout_retenu_eur
from public.acquisition_spikes asp
left join public.manual_entries me
  on me.account_id = asp.account_id
 and me.entity_type = 'acquisition_spike'
 and me.entity_key = asp.spike_start::text
 and me.field = 'budget_eur';

-- Les vues n'héritent pas des RLS des tables sous-jacentes en mode "invoker"
-- par défaut pour une vue simple sans security barrier explicite ; on force
-- security_invoker pour que les policies de acquisition_spikes/manual_entries
-- s'appliquent bien à l'appelant, pas au propriétaire de la vue.
alter view public.acquisition_spikes_with_budget set (security_invoker = true);

grant select on public.acquisition_spikes_with_budget to authenticated;
