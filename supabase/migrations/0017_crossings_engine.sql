-- Community Intelligence — Addendum v1.1 §5 — recompute_account() étendu.
--
-- Reprend intégralement la logique validée du Lot 3 (0010-0013 : épisodes,
-- exclusion des renommages des cohortes, médiane glissante par sous-requête
-- corrélée, persistance de cohort_survival via follower_states) et
-- l'étend selon l'ordonnancement imposé du §5 de l'addendum :
--   1. follower_states   2. following_states   3. cohorts + horizon commun
--   4. cohort_survival (troncature à horizon commun, §1)
--   5. hazard_curve (§3.4)   6. acquisition_spikes (§3.7)
--   7. content_attribution (§3.1)   8. cross_analyses + inflow_geo_estimate
--   9. reconciliation (§4)
-- L'étape 10 (real_cost_per_retained, §3.8) n'a pas de sous-étape SQL propre :
-- elle est portée par deux colonnes générées sur acquisition_spikes
-- (cout_brut_eur, cout_retenu_eur) qui se recalculent d'elles-mêmes à
-- chaque écriture de budget_eur, sans repasser par recompute_account —
-- c'est exactement ce que demande « recalculé aussi à chaque saisie de
-- budget ».
--
-- Déviations documentées (le PRD ne donne pas de formule fermée pour ces
-- points, seulement une intention) :
--  - cross_analyses.dimension : '' au lieu de NULL pour les croisements sans
--    axe, cf. commentaire de 0016 (contrainte unique effective).
--  - acquisition_spikes est recalculé en entier à chaque passage (comme
--    follower_states) : import_id est réécrit sur le dernier import à
--    chaque fois, donc la contrainte unique (account_id, import_id,
--    spike_start) ne peut pas servir de clé de conflit stable d'un
--    recalcul à l'autre. budget_eur (saisie humaine) est donc préservé par
--    spike_start dans une table temporaire avant le vidage, puis reporté
--    sur les lignes réinsérées — jamais perdu par le recalcul intégral.
--  - hazard_curve : âge = tenure_days déjà calculé par follower_states
--    (borne haute approx. pour les partis, âge courant pour les présents),
--    population episode = 1 uniquement (cohérent avec cohorts/cohort_survival).
--  - cohort_quality_score : les 4 facteurs sont normalisés min-max sur les
--    cohortes du compte ; si une seule cohorte existe (pas de comparaison
--    possible), le facteur ancienneté-au-départ vaut 50 (indéterminé) et les
--    facteurs déjà bornés [0,1] (survie, sans-signal, diurne) sont simplement
--    ramenés sur 100.
--  - inflow_geo : marge d'erreur = heuristique bornée [3,15] points,
--    proportionnelle au poids de l'ajustement départs-estimés dans le
--    calcul — ce n'est pas une dérivation statistique rigoureuse, l'addendum
--    ne demande d'ailleurs jamais mieux qu'`indicatif`.
--  - unfollow_boomerang : calculé sur la seule fenêtre de transition la plus
--    récente (following_states ne garde que le dernier état, sans épisode).
--  - territory_retention / format_retention / cadence_vs_churn dépendent de
--    `content` (Lot 5, pas encore ingéré) : tant qu'aucune ligne n'existe,
--    une seule ligne cross_analyses `insuffisant` est écrite avec la raison,
--    conformément à la règle transverse §0 (ne jamais masquer un résultat
--    faible).

alter table public.acquisition_spikes
  add column cout_brut_eur   numeric(12,2)
    generated always as (case when volume > 0 then round(budget_eur / volume, 2) end) stored,
  add column cout_retenu_eur numeric(12,2)
    generated always as (
      case when volume > 0 and retention_rate is not null and retention_rate > 0
        then round(budget_eur / (volume * retention_rate), 2)
      end
    ) stored;
...
begin
  ...
  -- Horizon commun (Addendum §1) : la plus courte exposition parmi les
  -- cohortes qui ont déjà traversé au moins un import après leur origine
  -- (une cohorte née au tout dernier import a une exposition quasi nulle et
  -- ne peut pas définir l'horizon de comparaison — elle sera simplement
  -- marquée `insuffisant`, cf. étape 4).
  ...
  if v_horizon_days is null then
    -- un seul import disponible : pas de comparaison possible, on retombe
    -- sur l'exposition de la cohorte unique (sera marquée insuffisant).
  ...
  -- ==========================================================
  -- 4. cohort_survival — série temporelle (inchangée du Lot 3, corrige le
  -- bug remaining/flip-flop via follower_states) + troncature à horizon
  -- commun (Addendum §1) sur la ligne du dernier import de mesure.
  -- ==========================================================
  with cohort_membership as (
    select profile_id, cohort_week, first_import_id as origin_import_id
    from public.follower_states where account_id = p_account_id and episode = 1
  ),
  pairs as (
    select cm.profile_id, cm.cohort_week, mi.id as measured_import_id, mi.exported_at as measured_at,
  ...
  remaining_flag as (
    select profile_id, cohort_week, measured_import_id, measured_at,
      (case
  ...
  insert into public.cohort_survival (
    ...
    horizon_days, rate_at_horizon, horizon_confidence
  )
  select
    ...
    -- « marquée insuffisant et non tronquée » (Addendum §1) : la troncature
    -- à horizon ne s'applique qu'aux cohortes dont l'exposition atteint H.
    case when a.measured_import_id = v_latest_import and a.exposure_days >= v_horizon_days
  ...
  -- ==========================================================
  -- 5. hazard_curve (§3.4) — table de vie discrète par tranche d'âge,
  -- population episode = 1, agrégat compte entier + détail par cohorte.
  -- ==========================================================
  ...
  population as (
    ...
    where account_id = p_account_id and episode = 1 and status in ('present','gone') and tenure_days is not null
  ),
  dep_bucket as (
  ...
  -- ==========================================================
  -- 6. acquisition_spikes (§3.7) — pic = jour où les arrivées quotidiennes
  -- (episode = 1) dépassent 3x la médiane glissante sur 14 jours ; les
  -- jours consécutifs en pic sont regroupés en un seul épisode. budget_eur,
  -- saisi à la main, est préservé à travers le recalcul (cf. plus bas).
  -- ==========================================================
  select
    count(*) filter (where sig_digit_suffix or sig_many_underscores or sig_long_handle)::numeric / nullif(count(*), 0),
    count(*) filter (where status = 'present')::numeric / nullif(count(*), 0)
  ...
  with daily as (
    ...
    from public.follower_states
    where account_id = p_account_id and episode = 1
  ...
  -- acquisition_spikes est recalculé en entier à chaque passage (comme
  -- follower_states), mais son import_id est réécrit sur le dernier import à
  -- chaque fois : la contrainte unique (account_id, import_id, spike_start)
  -- ne peut donc pas servir de clé de conflit stable d'un recalcul à
  -- l'autre (les anciennes lignes portent l'ancien import_id le plus
  -- récent, pas le nouveau). On préserve budget_eur par spike_start dans
  -- une table temporaire, on vide, puis on réinsère.
  ...
  -- ==========================================================
  -- 7. content_attribution (§3.1) — fenêtre de 48h après publication,
  -- ligne de base = médiane glissante 14 jours des arrivées quotidiennes.
  -- Ne produit rien tant que `content` est vide (Lot 5 non ingéré) : c'est
  -- le comportement correct, pas une lacune de cette étape.
  -- ==========================================================
  with daily as (
    select (followed_at at time zone 'UTC')::date as day, count(distinct profile_id) as arrivals
    from public.follower_states where account_id = p_account_id and episode = 1
  ...
  -- 3.2 territory_retention / 3.3 format_retention — dépendent de `content`.
  ...
  -- 3.5 cohort_quality_score — 4 facteurs normalisés min-max sur les
  -- cohortes du compte (survie à horizon, ancienneté médiane au départ,
  -- part sans signal structurel, part diurne Europe).
  with per_cohort as (
    select fs.cohort_week,
      ...
      (select percentile_cont(0.5) within group (order by fs2.tenure_days)
        ...
        where fs2.account_id = p_account_id and fs2.episode = 1 and fs2.cohort_week = fs.cohort_week and fs2.status = 'gone')::numeric as med_tenure_gone,
  ...
  normalized as (
    select pc.*,
      -- cohort_survival.rate_at_horizon stocke déjà un taux de SURVIE
      -- (1 - cohort_rate_at_horizon(), cf. migration précédente) : pas de
      -- second complément à 1 ici, ce serait une double inversion.
  ...
  -- 3.6 inflow_geo — inférence par différence de stocks entre les deux
  -- derniers imports portant une répartition géographique.
  if v_prev_import is not null then
    ...
    totals as (
      select
        ...
        (select count(*) from public.follower_states where account_id = p_account_id and status = 'gone' and departure_window_end = v_latest_at) as departures
    ),
    combined as (
    ...
    select
      ...
      -- pct_latest/pct_prev sont stockés sur une échelle 0-100 (cf.
      -- parsePercent) : chaque terme est ramené en effectif (/100 * total)
      -- avant la différence, puis le ratio est reconverti en points de
      -- pourcentage (*100) pour estimated_pct.
  ...
  -- 3.9 cadence_vs_churn — dépend de `content.published_at`, cf. le bloc
  -- « contenu non encore importé » ci-dessus.

  -- 3.10 reach_quality — portée non-abonnés vs qualité des cohortes entrantes.
  -- Une corrélation négative (portée non-abonnés en hausse, survie en
  -- baisse) est la lecture attendue par §3.10 si la portée achetée abîme
  -- la communauté. rate_at_horizon est un taux de SURVIE (cf. plus haut).
  with series as (
  ...
  -- 3.11 ecosystem_retention — comptes professionnels vs taux global.
  ...
  -- 3.12 unfollow_boomerang — sur la dernière transition de following_states
  -- uniquement (pas d'épisode conservé pour following_states).
  if v_prev_import is not null then
    with removed as (
      ...
      select r.profile_id,
        exists (
          select 1 from public.follower_states fl
           where fl.account_id = p_account_id and fl.profile_id = r.profile_id and fl.status = 'gone'
    ...
    reference as (
      select
        (select count(*) from public.follower_states where account_id = p_account_id and status = 'gone'
  ...
  -- ==========================================================
  -- 9. reconciliation (§4) — couverture arrivées/départs vs chiffres Meta.
  -- PK = import_id : une ligne par import de l'historique, pas seulement le
  -- dernier (cf. cohort_survival, même logique de série). Recalculée en
  -- entier à chaque passage à partir de audience_insights et
  -- follower_states, jamais accumulée par incrément.
  -- ==========================================================
  ...
  select
    ...
    round((select count(*) from public.follower_states where account_id = p_account_id and status = 'gone'
