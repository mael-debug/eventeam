-- Instrumentation fine de la section 1 (§4.10) : même isolée du reste
-- (0051), la section 1 (follower_states) dépasse 10min sur "Eden Park
-- All 2", alors que chacune de ses parties testées séparément est
-- rapide. Hypothèse : les tables temporaires tmp_follower_episodes et
-- tmp_episode_bounds (CTAS, jamais ANALYZE) et l'UPDATE renames (qui
-- tourne sur des stats de follower_states périmées, l'ANALYZE actuel
-- étant placé APRÈS lui) faussent les plans du planificateur — non
-- reproduit par des tests isolés sur des tables déjà peuplées/analysées.
-- Un RAISE LOG après chacune des 5 sous-étapes de la section 1 dira
-- laquelle consomme réellement le temps. Purement diagnostique.
...
begin
  ...
  raise log '[recompute_account %] t1a after tmp_follower_episodes %', p_account_id, clock_timestamp();
  ...
  raise log '[recompute_account %] t1b after tmp_episode_bounds %', p_account_id, clock_timestamp();
  ...
  raise log '[recompute_account %] t1c after follower_states insert %', p_account_id, clock_timestamp();
  ...
  raise log '[recompute_account %] t1d after renames update %', p_account_id, clock_timestamp();
