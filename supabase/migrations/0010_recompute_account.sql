-- Community Intelligence — Lot 3 — recompute_account()
-- PRD §4.2 (statut à trois valeurs dans la zone de recouvrement),
-- §4.3 (fenêtre de départ), §4.4 (cohortes et survie, exposure_days
-- obligatoire), §4.6 (renommages), §4.9 (pics d'acquisition), §4.10
-- (recalcul intégral, jamais incrémental).
--
-- Gère les épisodes (voir 0009_engine_schema.sql) : un profil qui part puis
-- revient avec un followed_at postérieur obtient une nouvelle ligne
-- (episode + 1), sans effacer l'épisode précédent.
...
begin
  ...
  -- ==========================================================
  -- Renommages présumés (§4.6) : un profil apparu pour la première fois
  -- (episode 1) à l'import B, avec un followed_at identique à la seconde
  -- près à celui d'un profil confirmé parti entre A et ce même B.
  -- ==========================================================
  with renames as (
    select
  ...
  -- ==========================================================
  -- cohorts — §4.4 (une cohorte = un groupe d'arrivée, episode = 1)
  -- ==========================================================
  ...
  -- ==========================================================
  -- cohort_survival — une ligne par (cohorte, import de mesure) depuis
  -- l'origine de la cohorte. "remaining" ne compte un départ que s'il est
  -- confirmé dans la zone de recouvrement (§4.2) ; un profil hors fenêtre
  -- reste compté comme "remaining" (indéterminé, jamais présumé parti).
  -- ==========================================================
  ...
  -- ==========================================================
  -- acquisition_spike — §4.9 : médiane glissante 14 jours sur les arrivées
  -- quotidiennes (episode = 1), pic si > 3x la médiane.
  -- ==========================================================
  with daily as (
