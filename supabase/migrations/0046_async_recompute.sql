-- Community Intelligence — sort recompute_account() du chemin synchrone de
-- process-import (§7.2).
--
-- Constat : recompute_account() rejoue l'intégralité de
-- follower_observations à chaque exécution ; sa durée croît avec
-- l'historique cumulé du compte (~45 s supplémentaires par import déjà
-- accumulé, mesuré sur Eden Park All). Même avec les correctifs 0042-0045,
-- l'exécuter en synchrone dans process-import (donc soumis au budget CPU de
-- l'Edge Function ET au statement_timeout Postgres) garantit une panne à
-- date inconnue : six mois d'exploitation mensuelle suffisent à dépasser
-- deux minutes, un an à quatre ou cinq. Ce n'est pas un problème de
-- performance à affiner davantage tant que le calcul reste sur le chemin
-- critique d'une requête HTTP.
--
-- Fix structurel : recompute_account() devient asynchrone.
--   1. process-import (modifié séparément) marque l'import 'computing' une
--      fois tous les fichiers parsés, SANS appeler recompute_account —
--      l'Edge Function répond immédiatement, son budget CPU n'est plus en
--      jeu.
--   2. Un job pg_cron (run_pending_recomputes, chaque minute) reprend tout
--      import 'computing', calcule ses analyses, puis bascule vers
--      'completed' (succès) ou 'failed' avec error_message (échec) — sans
--      aucun budget de temps externe : le calcul peut prendre dix minutes
--      sans que personne ne le remarque, et redevient rejouable au lieu
--      d'échouer en bloc (relancer = repasser l'import à 'computing').
--   3. recompute_account() doit donc considérer 'computing' comme un import
--      dont les données brutes sont prêtes (tous les fichiers sont déjà
--      'parsed' à ce stade), en plus de 'completed' — sinon le recalcul
--      ignorerait les propres observations de l'import qu'il est en train
--      de traiter.
--
-- Le statut 'completed' garde son sens strict : "import ET analyses
-- terminés" (cf. src/app/[org]/[brand]/layout.tsx, qui affiche la date du
-- dernier import status='completed' — ne doit PAS inclure 'computing').
...
-- ==========================================================
-- Job asynchrone : reprend chaque import 'computing', un compte à la fois,
-- protégé par un verrou consultatif pour ne jamais faire tourner deux
-- recalculs concurrents sur le même compte (un cycle de cron peut démarrer
-- avant que le précédent recalcul de ce même compte n'ait fini).
-- ==========================================================
...
revoke all on function public.run_pending_recomputes() from public, anon, authenticated;
grant execute on function public.run_pending_recomputes() to service_role;

create extension if not exists pg_cron;

select cron.schedule(
  'run-pending-recomputes',
  '* * * * *',
  $$select public.run_pending_recomputes();$$
);
