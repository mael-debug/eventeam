-- Correctif (§4.10) : l'UPDATE renames ne peut produire de résultat
-- non vide que par comparaison entre deux imports (il cherche des
-- profils au statut 'gone' issus d'un import précédent). Au premier
-- import d'un compte, follower_states ne contient que les abonnés du
-- fichier importé — aucune ligne 'gone' ne peut exister, le bloc
-- calcule donc nécessairement un résultat vide. Constaté sur le compte
-- @edenparkparis2 (79454559) : plusieurs minutes passées à produire du
-- vide. v_prev_import (calculé en tête de fonction) est déjà l'exact
-- indicateur de cette condition — non null seulement à partir du
-- second import. La requête renames elle-même n'est pas modifiée.
