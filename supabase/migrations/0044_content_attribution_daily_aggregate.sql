-- Community Intelligence — corrige *pour de vrai* le "canceling statement due
-- to statement timeout" de recompute_account() sur les gros comptes.
--
-- Migration 0043 remplaçait la sous-requête corrélée de retained_in_window
-- par un JOIN + GROUP BY, en supposant que la cause était le nombre de
-- sous-requêtes exécutées. C'était une fausse piste, invalidée par un test
-- contrôlé (EXPLAIN sur les mêmes données, avant/après ANALYZE explicite,
-- dans une seule transaction reproduisant exactement les conditions de
-- recompute_account()) :
--
--   - Le JOIN content x follower_states sur une condition BETWEEN (fenêtre
--     de date dépendant de la ligne de content) n'a PAS d'équi-jointure
--     possible : Postgres ne peut PAS le planifier en hash join ou merge
--     join, quels que soient les statistiques ou les réglages (vérifié avec
--     `set local enable_nestloop = off` : le planner garde un nested loop,
--     avec une pénalité de coût énorme, faute d'alternative). La réécriture
--     0043 ne change donc rien à l'algorithme réellement exécuté — seule la
--     syntaxe change.
--   - Le vrai problème : l'estimateur du planner se trompe fortement sur la
--     sélectivité de cette condition BETWEEN paramétrée sur l'index
--     d'expression (10551 lignes estimées par itération vs 89 lignes
--     réelles, un facteur ~118x). Avec un JOIN + GROUP BY, cette
--     sous-estimation peut faire matérialiser un produit intermédiaire
--     massif avant l'agrégation. Avec la sous-requête corrélée par ligne de
--     content (l'ancien pattern, et celui déjà utilisé pour arrivals_48h /
--     median_daily juste à côté dans la même fonction), chaque itération
--     agrège localement sans jamais matérialiser les paires — l'erreur
--     d'estimation ne coûte rien.
--
-- Fix définitif : calculer retained_in_window comme arrivals_48h et
-- median_daily le sont déjà — via une sous-requête corrélée sur une petite
-- table pré-agrégée par jour (daily_present, un par compte, quelques
-- centaines de lignes), jamais un JOIN direct sur follower_states (95k+
-- lignes) pour ce compte. Vérifié par EXPLAIN (ANALYZE, BUFFERS) réel sur
-- les données de production : 233 ms d'exécution, contre un plan à
-- 45 507 901 lignes estimées (donc plusieurs minutes en pratique) pour la
-- version 0043.
--
-- Reste identique au reste de la fonction (copie intégrale de la version
-- 0043, seule l'étape 7 change) — aucun autre comportement touché. Les
-- valeurs produites sont strictement identiques : daily_present ne fait que
-- pré-grouper le même filtre (account_id, episode = 1, status = 'present')
-- par jour avant de le sommer sur la fenêtre de 48h, au lieu de le
-- recompter à chaque ligne de content.
...
begin
  ...
  --
  -- retained_in_window : voir 0044 — jamais de JOIN/sous-requête corrélée
  -- directement sur follower_states (95k+ lignes pour un gros compte), le
  -- planner ne sait pas estimer correctement la sélectivité de la fenêtre de
  -- date paramétrée par ligne de content. daily_present pré-agrège le même
  -- filtre (episode=1, status='present') par jour une seule fois, puis
  -- chaque ligne de content ne fait qu'une petite sous-requête sur cette
  -- table de quelques centaines de lignes — exactement le même schéma que
  -- arrivals_48h / median_daily juste en dessous, déjà rapide.
  -- ==========================================================
  with daily as (
    select (followed_at at time zone 'UTC')::date as day, count(distinct profile_id) as arrivals
