-- content_attribution (étape 7) : `daily_present` n'est référencée qu'une
-- seule fois dans `pubs` (sous-requête corrélée `retained_in_window`).
-- N'étant référencée qu'une fois, PostgreSQL l'inline par défaut au lieu
-- de la matérialiser — la sous-requête corrélée recalcule alors un
-- GroupAggregate sur follower_states À CHAQUE publication au lieu de
-- scanner une seule fois la petite table pré-agrégée `daily_present`.
-- Confirmé par EXPLAIN sur le compte "Eden Park All 2" (2026-09-04) :
-- SubPlan 4 fait un Index Scan + GroupAggregate sur follower_states pour
-- chacune des 4313 publications, alors que `daily` (juste au-dessus,
-- référencée deux fois) reste correctement une CTE Scan bon marché.
-- C'est structurel — indépendant des statistiques ou du volume de
-- données déjà en place, donc reproductible avec ou sans ANALYZE à jour.
--
-- Correctif : forcer la matérialisation avec `AS MATERIALIZED`, comme
-- `daily` l'est déjà implicitement du fait de ses deux références.
