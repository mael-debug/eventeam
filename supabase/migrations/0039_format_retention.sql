-- Calcule enfin cross_analyses.format_retention (§3.3) — jusqu'ici un stub
-- qui n'insérait qu'une ligne "insuffisant" quand `content` était vide, et
-- ne calculait strictement rien quand du contenu existait (même bug de
-- classe que content_attribution.retention_rate, 0038). Contrairement à
-- territory_retention (thème éditorial, mise en scène) qui nécessiterait
-- une classification jamais construite, media_type (post/reel/story) est
-- une donnée réelle et déjà présente sur chaque ligne `content` — la
-- rétention moyenne par format se calcule directement à partir de
-- content_attribution.retention_rate, désormais renseigné (0038).
-- Le reste de recompute_account() est repris à l'identique — seule la
-- fin de la section 8 change.
...
begin
  ...
  pubs as (
    select c.id as content_id, c.published_at,
      ...
      (select count(*) filter (where fs.status = 'present')
         from public.follower_states fs
