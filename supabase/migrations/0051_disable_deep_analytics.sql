-- Changement de cap (§4.10) : l'instrumentation (0050) a montré que même
-- la section 1 (follower_states) dépasse largement les durées attendues
-- sur "Eden Park All 2" (>10min sans même atteindre la fin de la
-- section 1). Plutôt que de continuer à optimiser un calcul dont le
-- besoin réel ne justifie pas la complexité, on désactive les analyses
-- qui ne servent aucun des deux besoins produit actuels (comparaison
-- mensuelle abonnés, archivage des stats de publication déjà présentes
-- dans content_metrics) : cohortes hebdomadaires, survie, courbe de
-- départs par ancienneté, détection de pics, attribution d'abonnés aux
-- publications, cross_analyses, inflow_geo_estimate. Le code est
-- conservé derrière v_deep_analytics (constante à false) pour pouvoir
-- être réactivé plus tard — rien n'est supprimé, rien n'est optimisé.
--
-- Restent actives : section 1 (follower_states, la liste complète
-- depuis 2013), section 2 (following_states), section 9
-- (reconciliation). La section 1 reste potentiellement lente ; ce n'est
-- pas encore corrigé, seulement isolé du reste.
...
declare
  ...
  -- §4.10 : cohortes/survie/hazard/pics/attribution/cross_analyses/geo
  -- désactivées, ne servent aucun des deux besoins produit actuels. Code
  -- conservé, pas supprimé — remettre à true pour les réactiver.
  v_deep_analytics boolean := false;
begin
  raise log '[recompute_account %] t0 start %', p_account_id, clock_timestamp();
  ...
  raise log '[recompute_account %] t1 before section 1 (follower_states) %', p_account_id, clock_timestamp();
  ...
  -- ANALYZE explicite : voir 0045. Aucun autoanalyze ne peut s'exécuter au
  -- milieu de cette transaction ; sans ce rafraîchissement, les étapes
  -- suivantes choisissent un index non trié par date pour les agrégations
  -- par jour, forçant un tri complet au lieu d'un Incremental Sort quasi
  -- gratuit sur l'index 0042.
  analyze public.follower_states;

  raise log '[recompute_account %] t2 after section 1 (follower_states), before section 2 (following_states) %', p_account_id, clock_timestamp();
  ...
  raise log '[recompute_account %] t3 after section 2 (following_states), before section 3 (cohorts) %', p_account_id, clock_timestamp();

  if v_deep_analytics then
  ...
  raise log '[recompute_account %] t4 after section 3 (cohorts), before section 4 (cohort_survival) %', p_account_id, clock_timestamp();
  ...
  raise log '[recompute_account %] t5 after section 4 (cohort_survival), before section 5 (hazard_curve) %', p_account_id, clock_timestamp();
  ...
  raise log '[recompute_account %] t6 after section 5 (hazard_curve), before section 6 (acquisition_spikes) %', p_account_id, clock_timestamp();
  ...
  raise log '[recompute_account %] t7 after section 6 (acquisition_spikes), before section 7 (content_attribution) %', p_account_id, clock_timestamp();
  ...
  -- follower_states.
  --
  -- daily_present AS MATERIALIZED (0049) : référencée une seule fois dans
  -- `pubs`, elle serait sinon inlinée par défaut — la sous-requête
  -- corrélée recalculerait alors un GroupAggregate complet sur
  -- follower_states à CHAQUE publication (confirmé par EXPLAIN, 4313
  -- publications sur Eden Park All 2) au lieu de scanner une fois la
  -- petite table pré-agrégée, comme `daily` le fait déjà (elle, avec
  -- deux références, n'a jamais eu ce problème).
  -- ==========================================================
  with daily as (
    select (followed_at at time zone 'UTC')::date as day, count(distinct profile_id) as arrivals
  ...
  raise log '[recompute_account %] t8 after section 7 (content_attribution), before section 8 (cross_analyses) %', p_account_id, clock_timestamp();
  ...
  end if; -- v_deep_analytics (sections 3-8)

  raise log '[recompute_account %] t9 after section 8 (cross_analyses/geo/reach/ecosystem/boomerang), before section 9 (reconciliation) %', p_account_id, clock_timestamp();
  ...
  raise log '[recompute_account %] t10 after section 9 (reconciliation), end %', p_account_id, clock_timestamp();
